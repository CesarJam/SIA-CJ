<template>
  <div class="fade-in relative min-h-screen">
    ...
    <div class="space-y-4">
      ...
      <div v-for="user in listaUsuarios" :key="user.id"
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-3 mb-1">
            ...
            <span :class="{
              'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400': user.rol === 'admin',
              'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400': user.rol === 'cliente',
              'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400': user.rol === 'restringido'
            }" class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shrink-0">
              {{ user.rol === 'restringido' ? 'Restringido' : user.rol }}
            </span>
          </div>
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-wrap gap-1.5">
            <span v-if="!user.secciones_permitidas || user.secciones_permitidas.length === 0"
              class="text-xs text-gray-400 italic">
              Sin accesos
            </span>
            <span v-for="sec in user.secciones_permitidas" :key="sec"
        ...
        <div
          ...
          <button @click="abrirPanelEdicion(user)"
      ...
      <div v-if="listaUsuarios.length === 0"
        class="text-center py-10 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
    ...
    <div :class="['fixed inset-0 z-50 overflow-hidden', panelAbierto ? 'pointer-events-auto' : 'pointer-events-none']">
      ...
      <transition enter-active-class="ease-in-out duration-500" enter-from-class="opacity-0"
        ...
        <div v-if="panelAbierto" class="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity"
          @click="cerrarPanel"></div>
      </transition>
      ...
      <div class="absolute inset-y-0 right-0 max-w-full flex">
        <transition enter-active-class="transform transition ease-in-out duration-500 sm:duration-700"
          ...
          <div v-if="panelAbierto"
            class="relative w-screen max-w-md h-full flex flex-col bg-white dark:bg-gray-800 shadow-2xl">

            <div
              class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/40">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Datos
                  Principales</label>
                <div class="space-y-4">
                  <input v-model="form.email" type="email" placeholder="Correo de Google"
                  ...
                  <input v-model="form.nomenclaturaExpedienteArea" type="text"
                    placeholder="Nomenclatura (Ej: CJ/DA//2026)"
                    class="w-full px-4 py-3 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                  <select v-model="form.rol"
                    class="w-full px-4 py-3 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <option value="cliente">Cliente</option>
                    <option value="restringido">Usuario Restringido</option>
              ...
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-4">Secciones Permitidas (Lista
                  Blanca)</label>
                <div class="grid grid-cols-1 gap-2">
                  <label v-for="sec in todasLasSecciones" :key="sec.codigo"
                    class="flex items-center gap-3 p-3 rounded-lg border border-gray-100 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition-colors duration-200">
                    <input type="checkbox" :value="sec.codigo" v-model="form.secciones_permitidas"
                      class="rounded text-blue-600 focus:ring-blue-500 h-5 w-5 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600">
            ...
            <div class="p-6 border-t border-gray-200 dark:border-gray-700 flex gap-3 bg-gray-50 dark:bg-gray-900/40">
              <button @click="cerrarPanel"
                class="flex-1 py-2 text-gray-600 dark:text-gray-300 font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors">Cancelar</button>
              <button @click="guardarUsuario"
                class="flex-1 py-2 bg-blue-600 text-white rounded-lg font-bold shadow-md hover:bg-blue-700 hover:shadow-lg transform transition-all active:scale-95">
    ...
    <ConfirmModal :isOpen="modalEliminar.abierto" title="Eliminar Acceso"
